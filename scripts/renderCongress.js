...
const app = new Vue({
	...
	data: {
		...
		,{code: "AS", name: "American Samoa"}, {code: "GU", name: "Guam"}, {code: "MP", name: "Northern Mariana Islands"}, {code: "PR", name: "Puerto Rico"}, {code: "VI", name: "U.S. Virgin Islands"}]
	},
	computed: {
...
// todo: discard ascendingly and descendingly
const sortedOn = {
	firstNameAscendingly: true,
	firstNameDescendingly: false,
	lastNameAscendingly: false,
	lastNameDescendingly: false,
	partyAscendingly: false,
	partyDescendingly: false,
	stateAscendingly: false,
	stateDescendingly: false,
	districtAscendingly: false,
	districtDescendingly: false,
	seniorityAscendingly: false,
	seniorityDescendingly: false,
	partyVotesWithAscendingly: false,
	partyVotesWithDescendingly: false,
	partyVotesAgainstAscendingly: false,
	partyVotesAgainstDescendingly: false,
	setSortedOn: function(sortedType, sortMenuButton, sortButton, sortHeader, sortString){
		switch(sortedType){
			case 'firstName':
			if (this.firstNameAscendingly) {
				this.firstNameAscendingly = false;
				this.firstNameDescendingly = true;
			}else {
				this.firstNameAscendingly = true;
				this.firstNameDescendingly = false;
			}
			break;
			case 'lastName':
			if (this.lastNameAscendingly) {
				this.lastNameAscendingly = false;
				this.lastNameDescendingly = true;
			}else {
				this.lastNameAscendingly = true;
				this.lastNameDescendingly = false;
			}
			break;
			case 'party':
			if (this.partyAscendingly) {
				this.partyAscendingly = false;
				this.partyDescendingly = true;
			}else {
				this.partyAscendingly = true;
				this.partyDescendingly = false;
			}
			break;
			case 'state':
			if (this.stateAscendingly) {
				this.stateAscendingly = false;
				this.stateDescendingly = true;
			}else {
				this.stateAscendingly = true;
				this.stateDescendingly = false;
			}
			break;
			case 'district':
			if (this.districtAscendingly) {
				this.districtAscendingly = false;
				this.districtDescendingly = true;
			}else {
				this.districtAscendingly = true;
				this.districtDescendingly = false;
			}
			break;
			case 'seniority':
			if (this.seniorityAscendingly) {
				this.seniorityAscendingly = false;
				this.seniorityDescendingly = true;
			}else {
				this.seniorityAscendingly = true;
				this.seniorityDescendingly = false;
			}
			break;
			case 'partyVotesWith':
			if (this.partyVotesWithAscendingly) {
				this.partyVotesWithAscendingly = false;
				this.partyVotesWithDescendingly = true;
			}else {
				this.partyVotesWithAscendingly = true;
				this.partyVotesWithDescendingly = false;
			}
			break;
			case 'partyVotesAgainst':
			if (this.partyVotesAgainstAscendingly) {
				this.partyVotesAgainstAscendingly = false;
				this.partyVotesAgainstDescendingly = true;
			}else {
				this.partyVotesAgainstAscendingly = true;
				this.partyVotesAgainstDescendingly = false;
			}
			break;
		}
...
function setSortCardsListeners(){
	...
	sortByFirstName.onclick = () => {
		...
		let sortStringDescendingly = '<i class="fas fa-sort-alpha-down-alt"></i> Sort on first name';
		...
		if (sortedOn.firstNameAscendingly) {
			sortedOn.setSortedOn('firstName', sortMenuButton, sortByFirstName, undefined, sortStringDescendingly);
			filteredList.sort(sortByFirstNameDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('firstName', sortMenuButton, sortByFirstName, undefined, sortStringAscendingly);
	...
	sortByLastName.onclick = () => {
		...
		if (sortedOn.lastNameAscendingly) {
			sortedOn.setSortedOn('lastName', sortMenuButton, sortByLastName, undefined, sortStringDescendingly);
			filteredList.sort(sortByLastNameDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('lastName', sortMenuButton, sortByLastName, undefined, sortStringAscendingly);
	...
	sortByParty.onclick = () =>{
		...
		if (sortedOn.partyAscendingly) {
			sortedOn.setSortedOn('party', sortMenuButton, sortByParty, undefined, sortStringDescendingly);
			filteredList.sort(sortByPartyDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('party', sortMenuButton, sortByParty, undefined, sortStringAscendingly);
	...
	sortByState.onclick = () =>{
		...
		if (sortedOn.stateAscendingly) {
			sortedOn.setSortedOn('state', sortMenuButton, sortByState, undefined, sortStringDescendingly);
			filteredList.sort(sortByStateDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('state', sortMenuButton, sortByState, undefined, sortStringAscendingly);
	...
	sortBySeniority.onclick = () =>{
		...
		let sortStringDescendingly = '<i class="fas fa-sort-numeric-down-alt"></i> Sort on seniority';
		let filteredList = filter(searchByNameInput.value, partyFilterList, stateFilterList);

		if (sortedOn.seniorityAscendingly) {
			sortedOn.setSortedOn('seniority', sortMenuButton, sortBySeniority, undefined, sortStringDescendingly);
			filteredList.sort(sortBySeniorityDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('seniority', sortMenuButton, sortBySeniority, undefined, sortStringAscendingly);
	...
	sortByVotesWith.onclick = () =>{
		let sortStringAscendingly = '<i class="fas fa-sort-numeric-up"></i> Sort on votes with party';
		let sortStringDescendingly = '<i class="fas fa-sort-numeric-down-alt"></i> Sort on votes with party';
		let filteredList = filter(searchByNameInput.value, partyFilterList, stateFilterList);

		if (sortedOn.partyVotesWithAscendingly) {
			sortedOn.setSortedOn('partyVotesWith', sortMenuButton, sortByVotesWith, undefined, sortStringDescendingly);
			filteredList.sort(sortByVotesWithDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('partyVotesWith', sortMenuButton, sortByVotesWith, undefined, sortStringAscendingly);
	...
	sortByVotesAgainst.onclick = () =>{
		let sortStringAscendingly = '<i class="fas fa-sort-numeric-up"></i> Sort on votes against party';
		let sortStringDescendingly = '<i class="fas fa-sort-numeric-down-alt"></i> Sort on votes against party';
		let filteredList = filter(searchByNameInput.value, partyFilterList, stateFilterList);

		if (sortedOn.partyVotesAgainstAscendingly) {
			sortedOn.setSortedOn('partyVotesAgainst', sortMenuButton, sortByVotesAgainst, undefined
...
function setSortDistrictListener(){
	...
	if (sortByDistrict) {
		sortByDistrict.onclick = () =>{
			...
			let sortStringDescendingly = '<i class="fas fa-sort-alpha-down-alt"></i> Sort on district';
			let filteredList = filter(searchByNameInput.value, partyFilterList, stateFilterList);

			if (sortedOn.districtAscendingly) {
				sortedOn.setSortedOn('district', sortMenuButton, sortByDistrict, undefined, sortStringDescendingly);
				filteredList.sort(sortByDistrictDescending);
				updateVue(filteredList);
			}else {
				sortedOn.setSortedOn('district', sortMenuButton, sortByDistrict, undefined, sortStringAscendingly);
...
function setSortTableListeners(){
	...
	let sortHeaderArray = [nameHeader, partyHeader, stateHeader, seniorityHeader, votesWithHeader, votesAgainstHeader];
	...
	nameHeader.onclick = () =>{
		...
		if(sortedOn.firstNameAscendingly){
			sortedOn.setSortedOn('firstName', undefined, undefined, nameHeader, sortStringDescendingly);
			filteredList.sort(sortByFirstNameDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('firstName', undefined, undefined, nameHeader, sortStringAscendingly);
			filteredList.sort(sortByFirstNameAscending);
			updateVue(filteredList);
		}
	}
	...
	partyHeader.onclick = () =>{
		...
		setHeadersDefaultString(sortHeaderArray);
		if(sortedOn.partyAscendingly){
			sortedOn.setSortedOn('party', undefined, undefined, partyHeader, sortStringDescendingly);
			filteredList.sort(sortByPartyDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('party', undefined, undefined, partyHeader, sortStringAscendingly);
			filteredList.sort(sortByPartyAscending);
			updateVue(filteredList);
		}
	}
	...
	stateHeader.onclick = () => {
		...
		setHeadersDefaultString(sortHeaderArray);
		if (sortedOn.stateAscendingly) {
			sortedOn.setSortedOn('state', undefined, undefined, stateHeader, sortStringDescendingly);
			filteredList.sort(sortByStateDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('state', undefined, undefined, stateHeader, sortStringAscendingly);
			filteredList.sort(sortByStateAscending);
			updateVue(filteredList);
		}
	}
	...
	if (districtHeader){
		...
		districtHeader.onclick = () => {
			...
			setHeadersDefaultString(sortHeaderArray);
			if (sortedOn.districtAscendingly) {
				sortedOn.setSortedOn('district', undefined, undefined, districtHeader, sortStringDescendingly);
				filteredList.sort(sortByDistrictDescending);
				updateVue(filteredList);
			}else {
				sortedOn.setSortedOn('district', undefined, undefined, districtHeader, sortStringAscendingly);
				filteredList.sort(sortByDistrictAscending);
				updateVue(filteredList);
			}
		}
	...
	seniorityHeader.onclick = () => {
		...
		setHeadersDefaultString(sortHeaderArray);
		if (sortedOn.seniorityAscendingly) {
			sortedOn.setSortedOn('seniority', undefined, undefined, seniorityHeader, sortStringDescendingly);
			filteredList.sort(sortBySeniorityDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('seniority', undefined, undefined, seniorityHeader, sortStringAscendingly);
			filteredList.sort(sortBySeniorityAscending);
			updateVue(filteredList);
		}
	}
	...
	votesWithHeader.onclick = () => {
		...
		setHeadersDefaultString(sortHeaderArray);
		if (sortedOn.partyVotesWithAscendingly) {
			sortedOn.setSortedOn('partyVotesWith', undefined, undefined, votesWithHeader, sortStringDescendingly);
			filteredList.sort(sortByVotesWithDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('partyVotesWith', undefined, undefined, votesWithHeader, sortStringAscendingly);
			filteredList.sort(sortByVotesWithAscending);
			updateVue(filteredList);
		}
	}
	...
	votesAgainstHeader.onclick = () => {
		...
		setHeadersDefaultString(sortHeaderArray);
		if (sortedOn.partyVotesAgainstAscendingly) {
			sortedOn.setSortedOn('partyVotesAgainst', undefined, undefined, votesAgainstHeader, sortStringDescendingly);
			filteredList.sort(sortByVotesAgainstDescending);
			updateVue(filteredList);
		}else {
			sortedOn.setSortedOn('partyVotesAgainst', undefined, undefined, votesAgainstHeader, sortStringAscendingly);
			filteredList.sort(sortByVotesAgainstAscending);
			updateVue(filteredList);
		}
	}
...
function setHeadersDefaultString(sortHeaderArray){
	const sortDefaultStrings = ['Name', 'Party', 'State', 'Seniority', 'Votes with party'
	, 'Votes against party'];
