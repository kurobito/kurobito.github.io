...
const congressOverview = new Vue({
	...
	computed: {
		getCongressAtGlance: function() {
			if (chamber === "senate") {
				...
				this.congress.forEach(party => {
					party.pctVotedWith = (
						party.pctVotedWith / party.representatives
					).toFixed(2);
				});
		...
		congressTableHeaders: function() {
			...
			const pctVotedWithHeader = document.getElementById(
				"pctVotedWithHeader"
			);
	...
	methods: {
		incrementRepAndVoteWith: function(party, pctVotedWith) {
			...
			switch (party) {
				case "R":
					index = this.congress.findIndex(
						party => party.party === "Republican"
					);
					break;
				case "D":
					index = this.congress.findIndex(
		...
		sortByParty: function(event) {
			...
			const sortStringDescendingly =
				'Party <i class="fas fa-sort-down no-pointer-event">';
		...
		sortByNumOfReps: function(event) {
			const sortStringAscendingly =
				'# of representatives <i class="fas fa-sort-up no-pointer-event">';
			const sortStringDescendingly =
				'# of representatives <i class="fas fa-sort-down no-pointer-event">';
		...
		sortByPctVotedWith: function(event) {
			const sortStringAscendingly =
				'% voted with party <i class="fas fa-sort-up no-pointer-event">';
			const sortStringDescendingly =
				'% voted with party <i class="fas fa-sort-down no-pointer-event">';
